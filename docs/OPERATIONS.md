...
| `MAX_ACTIVE_SUBMISSIONS`                                          | no       | `200`                            | Service-wide (not per-submitter) ceiling on submissions that are queued or running at once. Checked before the row is inserted (`db.ts#insertSubmissionIfUnderCeiling`); a submission past it gets a 503 naming the current backlog size and the limit, never a silent drop. See "Resource ceilings" below.                                                                                          |
...
| `RATE_LIMIT_MAX` / `RATE_LIMIT_WINDOW_MS`                         | no       | `300` / `60000`                  | Global per-IP request limit across every route, via `@fastify/rate-limit`. Generous by design: the read endpoints must stay genuinely public and freely queryable. See "Resource ceilings" below.                                                                                                                                                                                                    |
| `SUBMISSIONS_RATE_LIMIT_MAX` / `SUBMISSIONS_RATE_LIMIT_WINDOW_MS` | no       | `5` / `60000`                    | Much stricter per-IP limit applied only to `POST /submissions`, since each accepted request triggers a real git clone and container build.                                                                                                                                                                                                                                                           |
...
accepted submission triggers a real git clone and a real, resource-consuming
isolated container build. Every ceiling below is checked before the
corresponding expensive work begins, not after, and every rejection is a
clear, structured error (429 or 503), never a silent drop:

- **Requests.** `SUBMISSIONS_RATE_LIMIT_MAX` / `SUBMISSIONS_RATE_LIMIT_WINDOW_MS`
  bound how often one IP can call `POST /submissions` (5/min by default) —
  `429 Too Many Requests`. `RATE_LIMIT_MAX` / `RATE_LIMIT_WINDOW_MS` apply a
  much more generous limit (300/min by default) to every route, including the
  read endpoints, so a future/unlisted route is never fully unprotected
  without making the read endpoints feel throttled for legitimate traffic.
- **Source size.** There is no byte-size check before the clone starts — the
  repository's size is not knowable without fetching it. Instead,
  `FETCH_TIMEOUT_MS` bounds the wall-clock time of the fetch container
  (`fetchSourceTarball`, `src/rebuild.ts`); a pathologically large or slow
  clone is killed on timeout and the job resolves `inconclusive`, rather than
  running unbounded.
- **Build duration.** `BUILD_TIMEOUT_MS` bounds the wall-clock time of the
  rebuild container (`runRebuild`, `src/rebuild.ts`); the container is killed
  on timeout. This already existed before rate limiting was added and is not
  duplicated by it.
- **Concurrent builds.** `JOB_CONCURRENCY` bounds how many jobs the runner
  processes _simultaneously_ (`src/queue.ts`). This already existed too.
- **Backlog.** `MAX_ACTIVE_SUBMISSIONS` bounds how many submissions can be
  queued-or-running at once, service-wide, regardless of who submitted them.
  This is the piece the other ceilings don't cover: a distributed set of
  submitters, each individually staying under the per-IP request limit above,
  could otherwise still grow the backlog (and eventually disk/DB usage as it
  drains) without bound. A submission past this ceiling gets `503 Service
Unavailable` naming the current backlog size and the limit.
...
- **Job backlog.** Watch the count of `pending`/`running` rows in the
  ...
  failing. A `503 queue_full` response means it reached `MAX_ACTIVE_SUBMISSIONS`
  (see "Resource ceilings" above) — a sustained run of these is the same
  signal as a growing backlog, just enforced at the door instead of observed
  after the fact.
